...
if [ -n "$BOOT_CFG" ]; then
    ...
    # Find the main config file that gets loaded
    MAIN_CFG=$(find "${ISO_DIR}" -path "*/sysresccd.cfg" 2>/dev/null | head -1)
    
    if [ -n "$MAIN_CFG" ]; then
        echo "Found main config: ${MAIN_CFG}"
        cp "${MAIN_CFG}" "${MAIN_CFG}.bak"
        
        # Modify the first menu entry to include our autorun script
        # Look for the first APPEND line and add our script
        sed -i '0,/APPEND archisobasedir=/s|APPEND archisobasedir=|APPEND archisobasedir=sysresccd script=/autorun/serverstore-init.sh archisobasedir=|' "${MAIN_CFG}"
        
        # Also set it as default and reduce timeout
        sed -i 's/^MENU DEFAULT$//' "${MAIN_CFG}"
        sed -i '0,/^LABEL/s/^LABEL/MENU DEFAULT\nLABEL/' "${MAIN_CFG}"
        
        echo "Main config updated with autorun script"
    fi
...
# Create autorun directory and script
...
if ! command -v smartctl &> /dev/null; then
    pacman -Sy --noconfirm smartmontools 2>/dev/null
fi
...
if [ -f /run/archiso/bootmnt/serverstore/dashboard.py ]; then
    ...
    echo "Warning: Dashboard not found on ISO"
fi

# Start the dashboard in background
if [ -f /opt/serverstore/dashboard.py ]; then
    echo "Starting ServerStore dashboard..."
    nohup /usr/bin/python /opt/serverstore/dashboard.py > /var/log/serverstore.log 2>&1 &
    sleep 2
    ...
    if pgrep -f "dashboard.py" > /dev/null; then
        ...
        echo "Warning: Dashboard failed to start. Check /var/log/serverstore.log"
    fi
fi
...
echo "Login: root (no password needed)"
echo ""
echo "To check dashboard status: ps aux | grep dashboard"
echo "To view dashboard logs: tail -f /var/log/serverstore.log"
echo ""
